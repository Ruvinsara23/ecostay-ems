...
| Savings | derived | **Headline: evidence-armed avoided energy (ADR-0015)** — a circuit earns Σ(rated wattage × time) only where the recorded samples show it was commanded on while the occupancy policy allowed it to run, and then blocked once the guest left. Credited in `EXIT_PENDING`/`VACANT_CONFIRMED`, capped per continuous vacancy, priced via the tariff engine at the marginal band rate. A room nobody occupied earns nothing. Sleep suspension is excluded. **Secondary: kWh per occupied-hour** trend from daily aggregates. |
| Armed circuit | derived | A controlled circuit the samples prove was energised during the guest's stay — the precondition for any savings credit (ADR-0015). Missing relay data cannot arm a circuit. |
| Credited vacancy | derived | The bounded span of `EXIT_PENDING`/`VACANT_CONFIRMED` for which an armed circuit earns avoided energy. Capped at `MAX_CREDIT_MS_PER_VACANCY`; the cap resets when the guest returns. |
