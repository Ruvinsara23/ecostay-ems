...
- ADR-0014 uses a per-device sleep policy. Sleep suspends lights/fan while a
  commanded AC remains on; retained light/fan intent resumes after confirmed
...
and clears commands on confirmed exit even when server automation is OFF, so the legacy
...
| **Device control** | Owner toggles lights, exhaust fan, AC, water pump, and presence relay; `mainRelay` is excluded. Entry, exit pending, and vacancy block all comfort controls. Exit pending retains settings for immediate cancelled-exit recovery; confirmed vacancy clears them. Sleep blocks lights/fan but keeps commanded AC available, pump stays independent, and gas may force the fan. | `src/rooms/room-live-view.tsx` (DeviceControls) |
